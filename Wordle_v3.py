...
def shortlist_words2(combo, dist_df):
    combo = list(combo)
    curr_det = []
    curr_det.extend(combo)
    #dist_df = dist_matrix(combo)
    curr_dist = dist_df.to_numpy().sum()/2
    curr_det.append(curr_dist)
    return curr_det
...
new_gmm_df = gmm_df[gmm_df.maxValues > 0.98]
#new_gmm_df.groupby('labels', group_keys=False).apply(lambda x: x.sample(min(len(x), 2)))
new_gmm_df = new_gmm_df.groupby('labels', group_keys=False).apply(lambda x: x.sample(frac=0.1))
new_gmm_df.reset_index(inplace = True)

shortlist_df = df[df.index.isin(new_gmm_df.words)]
print("number of shortlist", shortlist_df.shape[0])

points = shortlist_df.values
#Getting Hull vauues
hull = ConvexHull(points)
vertice_id = list(hull.vertices)
print("number of hullpoints", len(vertice_id))
#lw = list(new_gmm_df.words)
#vertice_words = [lw[i] for i in lw]

print(1, new_gmm_df.shape[0])
new_gmm_df = new_gmm_df.loc[new_gmm_df.index.isin(vertice_id)]
print(2, new_gmm_df.shape[0])

final_combo_list_master =[]
uniq =[]
stratified_df = new_gmm_df.sample(frac=0.2, replace=True, random_state=1)
